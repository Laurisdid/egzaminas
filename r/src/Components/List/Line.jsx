...
function Line({ line }) {
    ...
    const confirm = () => {
        ...
        console.log({rank:parseInt(rank),id: line.id});
        axios.put('http://localhost:3003/meister/rate/',{ rank:parseInt(rank),id: line.id});
    ...
    return (
        <li className="list-group-item">
            <div className="item">
                ...
                <b style={{ paddingRight: "9%" }}>{line.meistras}</b>
                    <i style={{ padding: "3%"}}>{line.vardas} </i>
                    <i style={{ padding: "3%"  }}>{line.pavarde} </i>
                    <i style={{ padding: "3%"  }}>{line.spec} </i>
                    <i style={{ padding: "3%" }}>{line.servisas} </i>
                    <i style={{ padding: "3%"}}>{line.miestas} </i>
                    <i style={{ padding: "3%"}}>{line.rank} </i>
                    {
                ...
                <div className="form-group" style={{ float: "right",width: "100px" }}>
                    <input type="text" style={{ width: "80px"}}onChange={e => setRank(e.target.value)} value={rank} />
